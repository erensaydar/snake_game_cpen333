# Name: Cameron Taylor, Eren Saydar
# Student number: 78955465, 15764186
...
class Game():
    ...
    def __init__(self, queue):
        ...
        self.direction = "Left"
        #initial location of the prey (added by the students)
        self.rectangleCoordinates = (0,0,0,0)
    ...
    def move(self) -> None: 
        """ 
            ...
            and position) should be correctly updated. #new coordinate will be the new last tuple in the list, all the other ones will take the value of their successor
        """
        NewSnakeCoordinates = self.calculateNewCoordinates() #this is only a tuple for the new last member of the snake coordinates list
        #complete the method implementation below

        for i in range(len(self.snakeCoordinates)-1): #loop to change the coordinates of the snake 
            self.snakeCoordinates[i] = self.snakeCoordinates[i+1]
        
        self.snakeCoordinates[len(self.snakeCoordinates)- 1] = NewSnakeCoordinates #the last coordinate will be changed to the new coordinate
        self.queue.put({"move":self.snakeCoordinates})
        
        self.isGameOver(self.snakeCoordinates)
        
        #(x value of head coordinate of snake - x value of middle of prey) and (y value of head - middle y value of prey) has to be less than or equal to 5 because prey is a 10x10 square
        if(abs(self.snakeCoordinates[len(self.snakeCoordinates)-1][0]-(self.rectangleCoordinates[0]+5)) <= 5 and
            abs(self.snakeCoordinates[len(self.snakeCoordinates)-1][1]-(self.rectangleCoordinates[1]+5)) <= 5): #formula to check if snake eats the prey or not
            self.score += 1
            self.queue.put({"score":self.score})
            self.snakeCoordinates.append(NewSnakeCoordinates) #inreases the length of the snake by adding another tuple to the list
            self.createNewPrey()
    ...
    def calculateNewCoordinates(self) -> tuple: 
        ...
        """
        lastX, lastY = self.snakeCoordinates[-1] #-1 index means you count from right so this refers to last element(last tuple)
        #complete the method implementation below
        if (self.direction == "Left"):
            lastX -= 10 #the tuples in the snake coordinates lists were initiated with 10 digit difference
    ...
    def isGameOver(self, snakeCoordinates) -> None: 
        ...
        """
        x, y = snakeCoordinates[-1] #coordinates of the head of the snake
        
        if(x < 10 or x > WINDOW_WIDTH - 10 or y < 10 or y > WINDOW_HEIGHT - 10 ): # formula to check if the snake hits the borders (walls)
            self.gameNotOver = False
            self.queue.put({"game_over":True})

        for i in range(len(snakeCoordinates)-1): #loop to check if the snake bites itself
            if(abs(snakeCoordinates[-1][0]-snakeCoordinates[i][0]) <= 5 and 
               abs(snakeCoordinates[-1][1]-snakeCoordinates[i][1]) <= 5):
               self.gameNotOver = False
               self.queue.put({"game_over":True})
